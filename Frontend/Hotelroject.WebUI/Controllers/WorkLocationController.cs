...
using Hotelroject.WebUI.Dtos.WorkLocationDto;
...
namespace Hotelroject.WebUI.Controllers
{
    public class WorkLocationController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5191/api/WorkLocation?id={id}");
        ...
        public async Task<IActionResult> UpdateWorkLocation(UpdateWorkLocationDto updateWorkLocationDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:5191/api/WorkLocation/", stringContent);
